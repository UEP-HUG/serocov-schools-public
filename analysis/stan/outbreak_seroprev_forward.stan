...
functions {
  ...
  real to_integrate(
    ...
    real to_integrate2(
      ...
      if (size(x_r) == 1) {
        print("Integrating with mu: ", mu, " sigma: ", sigma, " scale: ", x_r[1]);
  ...
  for(i in 1:M) {
    ...
    for (j in starts[i]:ends[i]) {
      ...
      if (o == 0) {
        ...
        hz = eta_int[j] * dt[j]/dt_scale + exp(log_alpha[o] + alpha_i_raw[q] * alpha_i_sd[o]);
      }
...
model {
  ...
  for (i in 1:N_all_tests) {
    ...
    if (g == 0) {
      ...
      hz = eta_int[l] * inf_window/dt_scale + exp(log_alpha[g] + alpha_i_raw[q] + alpha_i_sd[g]);
    }

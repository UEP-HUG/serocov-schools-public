...
functions {
  ...
  real to_integrate(
    ...
    real to_integrate2(
      ...
      if (size(x_r) == 1) {
        res = exp(-exp(mu + sigma * inv_Phi(x)) * x_r[1]);
      } else {
        res = exp(-exp(mu + sigma * inv_Phi(x)));
      }
      ...
      // To integrate out variability
      real to_integrate2d_upper(
        real x,          // Function argument
        real xc,         // Complement of function argument
        //  on the domain (defined later)
        array[] real theta,    // parameters size 3, with mu, sigma1 and sigma2
        data array[] real x_r,      // data (real)  size of 1 with tolerance
        data array[] int x_i) {     // data (integer) empty
        
        real res;
        //real tol = x_r[1];
        array[0] real x_r_lower;    // this is the second variable to pass to lower integral
        array[1] real y;
        y[1] = x;
        
        // x_r_lower[1] = x;
        res = integrate_1d(to_integrate2d_lower, 0, 1, append_array(theta, y), x_r, x_i, 10);
        
        return res;
        }
        
        real to_integrate2d_lower(
          real x,          // Function argument
          real xc,         // Complement of function argument
          //  on the domain (defined later)
          array[] real theta,    // parameters
          ...
          res = exp(-exp(mu + sigma1 * inv_Phi(x) + sigma2 * inv_Phi(y)));
          return res;
          }
...
data {
  ...
  array[V] int<lower=1, upper=Z_int_comb> map_outbreak_int_variant;    // indices of the community transmission parameters during in-period schooling
  ...
  real mu_mu_log_alpha;             // hyperpriors on the mean and sd of outbreak infection prob
  real<lower=0> sd_mu_log_alpha;
...
parameters {
  ...
  // Outbreak exposure
  vector[V] mu_log_alpha;
  vector<lower=0>[V] sd_log_alpha;
  vector[G] log_alpha_tilde;
  vector[L_outbreaks] alpha_i_raw;    // individual level outbreak random efects
  vector<lower=0>[V] alpha_i_sd;      // sd of individual level random effects (REs)
...
transformed parameters {
  ...
  eta_int = exp(X_int * beta_int + alpha_i_int_raw .* alpha_i_int_sd[map_int_int_comb]);
  ...
  // Compute the pooled estimates of outbreak infection probability
  log_alpha = mu_log_alpha[map_outbreak_variant] + sd_log_alpha[map_outbreak_variant] .* log_alpha_tilde;
  ...
  for(i in 1:M) {
    ...
    for (j in starts[i]:ends[i]) {
      ...
      if (o == 0) {
        ...
        hz = eta_int[j] * dt[j]/dt_scale + exp(log_alpha[o] + alpha_i_raw[q] * alpha_i_sd[v]);
      }
...
model {
  ...
  for (i in 1:N_all_tests) {
    ...
    if (g == 0) {
      ...
      hz = eta_int[l] * inf_window/dt_scale + exp(log_alpha[g] + alpha_i_raw[q] + alpha_i_sd[v]);
    }
  ...
  alpha_i_int_raw ~ std_normal();
  alpha_i_int_sd ~ normal(0, 2.5);
  
  // ---- Mod.C.2: Outbreak infections ----
  mu_log_alpha ~ normal(mu_mu_log_alpha, sd_mu_log_alpha);
  sd_log_alpha ~ normal(0, 5);
  log_alpha_tilde ~ std_normal();
  
  alpha_i_raw ~ std_normal();
  alpha_i_sd ~ normal(0, 2.5);
  ...
  (t_log_500 * beta[1:4]) ~ normal(0, 1);  // logit-scale, inv_logit(1) ~= 0.75
...
generated quantities {
  vector[Z_int_comb] p_week_community;     // weekly infection probability from community transmission
  vector[G] p_outbreak;                    // probability of infeciton during outbreak accounting for community infections
  vector[V] p_outbreak_variant;            // pooled mean probability of infection during outbreaks by variant
  matrix[N_sens_pred, max(Z_comb, 1)] sens_pred;    // predicted time-varying sensitivity of serology
  vector[N] gen_sero;                               // random draws from serology results
  vector[E] gen_sero_event_all;                     // random draws from serology results by event across all participants
  vector[E] gen_sero_event_obs;                     // random draws from serology results by event for sampled participants (to compare to data)
  vector[E] gen_seroprev_event_all;                 // seroprevalence corresponding to random serology result draws
  vector[E] gen_seroprev_event_obs;                 // seroprevalence corresponding to random serology result draws
  real default_sens = inv_logit(t_log_default * beta[1:4]);
  ...
  for (i in 1:G) {
    ...
    p_outbreak[i] = 1 - integrate_1d(to_integrate2, 0, 1, par_c, x_r_c, x_i, tol) * 
    integrate_1d(to_integrate2, 0, 1, par_o, x_r_o, x_i, tol);
  }
  
  
  // By variant (pooled means)
  for (i in 1:V) {
    int j = map_outbreak_int_variant[i];
    array[1] real x_r_c;
    array[1] real x_r_o;
    array[0] int x_i;
    array[2] real par_c;
    array[3] real par_o;
    par_c[1] = X_int_comb[j, ] * beta_int;
    par_c[2] = alpha_i_int_sd[j];
    par_o[1] = mu_log_alpha[i];
    par_o[2] = sd_log_alpha[i];
    par_o[3] = alpha_i_sd[i];
    x_r_c[1] = inf_window/dt_scale;    // 7 days for weekly estimates
    x_r_o[1] = tol;
    p_outbreak_variant[i] = 1 - integrate_1d(to_integrate2, 0, 1, par_c, x_r_c, x_i, tol) *
    integrate_1d(to_integrate2d_upper, 0, 1, par_o, x_r_o, x_i, 10);
  }
